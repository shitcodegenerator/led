...
<template>
  <div class="gap-4 flex flex-col mb-12 overflow-y-scroll max-h-[70vh] sm:max-h-[55vh]">
    <div v-for="i in form" class="flex sm:flex-row flex-col gap-1 sm:gap-4 
items-start sm:items-center">
      <label v-if="i.type !== 'photo'" class="text-[#666666] flex-1">{{ i.label }}</label>
      <input v-if="i.type !== 'photo'" v-model="i.value" :placeholder="i.placeholder" :type="i.type ?? 'text'"/>

      <div v-if="i.type == 'photo'" class="flex sm:flex-row flex-col gap-4">
        <label class="text-[#666666] flex-1 w-[162px]">{{ i.label }}</label>
          <div  class="flex flex-col relative border-[#d9d9d9] border rounded h-[135px] w-[208px] flex items-center justify-center">
      ...
      <div class="enroll">
        <div class="flex flex-row items-center justify-center bg-[#d9d9d9] py-2 text-dark font-bold text-center hover:opacity-80 cursor-pointer rounded-t" @click="hide">活動注意事項
            <img :src="downIcon"/>
        </div>
...
input:not(.checkbox) {
  @apply flex-[2] border pl-4 border-[#d9d9d9] rounded-sm h-20 sm:h-9 min-w-[208px] border-solid sm:w-auto w-full sm:py-0 py-2;
}
